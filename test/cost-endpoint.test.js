 ...
 *   GET /model-chooser/cost
 *   GET /model-chooser/cost-history
 *
...
test("cost-history reads the private log through snapshotEvents()", () => {
  ...
  apply(ctx);
  const handler = routes.get("/model-chooser/cost-history");
  assert.ok(handler, "cost-history route registered");
  const { status, body } = request(handler, "/model-chooser/cost-history?session=s1&limit=200");
...
test("cost aggregates the same log instead of returning zeros", () => {
  const { ctx, routes } = makeCtx({ s1: modernSession(USAGE_EVENTS) });
  apply(ctx);
  const { status, body } = request(routes.get("/model-chooser/cost"), "/model-chooser/cost?session=s1");
  assert.equal(status, 200);
...
test("an unreadable facade degrades to zeros instead of throwing", () => {
  ...
  apply(ctx);
  const { status, body } = request(routes.get("/model-chooser/cost-history"), "/model-chooser/cost-history?session=s1");
  assert.equal(status, 200);
...
test("cost routes answer 400 without a session and 404 for an unknown one", () => {
  const { ctx, routes } = makeCtx({ s1: modernSession(USAGE_EVENTS) });
  apply(ctx);
  assert.equal(request(routes.get("/model-chooser/cost"), "/model-chooser/cost").status, 400);
  assert.equal(request(routes.get("/model-chooser/cost-history"), "/model-chooser/cost-history?session=nope").status, 404);
});
