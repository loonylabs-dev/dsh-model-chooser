...
test("POST /model-chooser/refresh-models merges live ids and writes lists", async () => {
  const providers = {
  ...
  try {
    ...
    apply(ctx);
    const handler = routes.get("/model-chooser/refresh-models");
...
test("server-models resolves local gateway keys via the credentials service", async () => {
  ...
  try {
    ...
    apply(ctx);
    const handler = routes.get("/model-chooser/server-models");
