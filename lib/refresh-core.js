/**
 * model-chooser refresh core — discovery and merge for provider model lists.
 *
 ...
 * @returns the model-chooser entry for the settings `models` list.
 */
...
export async function discoverProvider({
	...
	if (endpoint === undefined)
		return { ok: false, error: "no endpoint (neither baseURL nor pi-ai catalog)" };
	...
	try {
		...
		if (records === undefined || records.length === 0)
			return { ok: false, error: "response carried no model list" };
...
export function summarize(results) {
	...
	const parts = [
		`${ok.length} providers, ${total} models`,
		added > 0 ? `+${added} new` : "",
		removed > 0 ? `−${removed} removed` : "",
	].filter(Boolean);
	if (failed > 0) parts.push(`${failed} providers failed`);
	return parts.join(" · ");
}
