/**
 * model-chooser — host carrier + live cost endpoint.
 *
...
// ─── Model-list refresh (POST /model-chooser/refresh-models) ─────────────────
//
...
async function runModelRefresh(ctx) {
  ...
  return {
    results: rows,
    summary: summarize(results) + (written > 0 ? ' · ' + written + ' lists written' : ''),
...
export function apply(ctx) {
  ...
  const mount = () => {
    ...
    }), 'model-chooser: /model-chooser/cost route')
    ctx.effect(() => webServer.register({
      kind: 'exact',
      path: '/model-chooser/catalog',
      handler: (req, res) => {
    ...
    }), 'model-chooser: /model-chooser/catalog route')
    ctx.effect(() => webServer.register({
      kind: 'exact',
      path: '/model-chooser/cost-history',
      handler: (req, res) => {
        const url = new URL(req.url ?? '', 'http://127.0.0.1')
        const sessionId = url.searchParams.get('session')
        if (!sessionId) return writeJson(res, 400, { error: 'missing session' })
    ...
    }), 'model-chooser: /model-chooser/cost-history route')
    ctx.effect(() => webServer.register({
      kind: 'exact',
      path: '/model-chooser/server-models',
    ...
    }), 'model-chooser: /model-chooser/server-models route')
    ctx.effect(() => webServer.register({
      kind: 'exact',
      path: '/model-chooser/refresh-models',
      handler: async (req, res) => {
    ...
    }), 'model-chooser: /model-chooser/refresh-models route')
  }
  mount()
